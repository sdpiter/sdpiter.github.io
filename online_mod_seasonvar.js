...
(function () {
    ...
    function seasonvarHost() {
      return 'https://seasonvar.ru';
    }

    function seasonvarHost2() {
      return 'https://www.seasonvar-enter.ru';
    }

    function seasonvarProxy() {
      var CORS_PROXIES = [
        'https://cors.nb557.workers.dev/',
        'https://cors.fx666.workers.dev/',
        'https://cors557.deno.dev/'
      ];
      return CORS_PROXIES[Math.floor(Date.now() / 3600000) % CORS_PROXIES.length];
    }
    ...
    function proxy(name) {
      ...
      if (name === 'seasonvar') return user_proxy1;
    ...
    var Utils = {
      ...
      seasonvarHost: seasonvarHost,
      seasonvarHost2: seasonvarHost2,
      seasonvarProxy: seasonvarProxy,
...
﻿    function seasonvar(component, _object) {
      ...
      this.search = function (_object, kinopoisk_id) {
        ...
        apiAutocomplete(select_title, function (results) {
      ...
      function apiAutocomplete(query, onSuccess, onError) {
        var url = host + '/autocomplete.php?query=' + encodeURIComponent(query);
        network.clear();
        network.timeout(10000);
        network['native'](component.proxyLink(url, prox, prox_enc, 'enc2t'), function (str) {
          try {
            var json = typeof str === 'string' ? JSON.parse(str.data || str) : str;
            var results = [];
            if (json && json.data) {
              for (var i = 0; i < json.data.length; i++) {
                var path = json.data[i];
                if (!path || /\/+/.test(path)) continue;
                var title = json.suggestions && json.suggestions.valu ? json.suggestions.valu[i] : path;
      ...
      function pickBestResult(results) {
        ...
        if (!best) {
          for (var j = 0; j < results.length; j++) {
            var r2 = results[j];
            if (normalizeTitle(r2.title).indexOf(normalizeTitle(select_title)) !== -1) { best = r2; break; }
          }
        }
      ...
      function normalizeTitle(str) {
        return (str || '').toLowerCase().replace(/ё/g, 'е').replace(/[^a-zа-яе0-9\s]/g, '').replace(/\s+/g, ' ').trim();
      }

      function loadSerial(serialInfo) {
        component.loading(true);
        var serialUrl = serialInfo.url;
        if (!serialUrl && serialInfo.path) serialUrl = host + serialInfo.path;
        if (!serialUrl) { component.emptyForQuery(select_title); return; }

        network.clear();
        network.timeout(15000);
        network['native'](component.proxyLink(serialUrl, prox, prox_enc, 'enc2t'), function (html) {
          ...
          var secureMark = secureMatch ? secureMatch[1] + '0' : '';
          var serialId = idMatch ? idMatch[1] : serialInfo.id || '';
          ...
          while ((m = re.exec(html)) !== null) {
            ...
            var found = false;
            for (var i = 0; i < seasonLinks.length; i++) { if (seasonLinks[i].num === sNum) { found = true; break; } }
            if (!found) seasonLinks.push({ num: sNum, path: m[1] });
          }

          var plMatch = html.match(/pl\/playlist_(\d+)\.js/);
          var curSeason = parseInt((html.match(/class="sel-season[^>]*data-season="(\d+)"/) || [])[1] || '1', 10);

          if (!seasonLinks.length && !plMatch) {
            var navRe = /<a[^>]+href="([^"]*season\.html)"[^>]*>Сезон\s*(\d+)/g;
            while ((m = navRe.exec(html)) !== null) {
              var n = parseInt(m[2], 10);
              var f2 = false;
              for (var j = 0; j < seasonLinks.length; j++) { if (seasonLinks[j].num === n) { f2 = true; break; } }
              if (!f2) seasonLinks.push({ num: n, path: m[1] });
            }
          }

          if (plMatch) { loadPlaylist(plMatch[1], serialInfo, curSeason); return; }
          ...
          if (seasonLinks.length > 0) {
            ...
            function checkDone() {
              if (--pending === 0) {
                sData.sort(function (a, b) { return a.season - b.season; });
                parseSeasons(sData, serialInfo);
              }
            }
            seasonLinks.forEach(function (sL) {
              var sUrl = host + sL.path;
              var net2 = new Lampa.Reguest();
              net2.timeout(12000);
              net2['native'](component.proxyLink(sUrl, prox, prox_enc, 'enc2t'), function (sHtml) {
                if (typeof sHtml !== 'string') sHtml = (sHtml && (sHtml.body || sHtml.data || '')) + '';
                var eps = scrapeEpisodes(sHtml, sL.num);
                sData.push({ season: sL.num, title: 'Сезон ' + sL.num, episodes: eps });
                checkDone();
              }, function () {
                sData.push({ season: sL.num, title: 'Сезон ' + sL.num, episodes: [] });
                checkDone();
              }, false, { dataType: 'text' });
            });
          } else if (secureMark && serialId) {
      ...
      function loadPlaylist(plId, serialInfo, curSeason) {
        var url = host + '/pl/playlist_' + plId + '.js';
        network.clear(); network.timeout(10000);
        network['native'](component.proxyLink(url, prox, prox_enc, 'enc2t'), function (data) {
          var items = [];
          if (Array.isArray(data)) {
            data.forEach(function (ep, i) {
              items.push({ title: ep.title || ep.comment || ('Серия ' + (i + 1)), episode: i + 1, file: ep.file || ep.link || '', type: 'file' });
            });
      ...
      function loadXmlPlaylist(secureMark, serialId, serialInfo) {
        var url = host + '/playls2/' + secureMark + '/trans/' + serialId + '/list.xml';
        network.clear(); network.timeout(10000);
        network['native'](component.proxyLink(url, prox, prox_enc, 'enc2t'), function (xmlText) {
          var episodes = [];
          try {
            var parser = new DOMParser();
            var xmlDoc = parser.parseFromString(xmlText, 'text/xml');
            var plEl = xmlDoc.getElementsByTagName('playlist')[0];
            if (plEl) {
              var xmlItems = plEl.children;
              for (var i = 0; i < xmlItems.length; i++) {
                if (xmlItems[i].tagName !== 'item') continue;
                var fEl = xmlItems[i].getElementsByTagName('file')[0];
                var cEl = xmlItems[i].getElementsByTagName('comment')[0];
                if (fEl && fEl.textContent) {
                  episodes.push({ title: cEl ? cEl.textContent.trim() : ('Серия ' + (i + 1)), episode: i + 1, file: fEl.textContent.trim(), type: 'file' });
                }
              }
            }
          } catch (e) {}
      ...
      function scrapeEpisodes(html, seasonNum) {
        ...
        var plMatch = html.match(/pl\/playlist_(\d+)\.js/);
        if (plMatch) {
          episodes.push({ title: 'Сезон ' + seasonNum, episode: 0, playlist_id: plMatch[1], type: 'playlist' });
          return episodes;
        }
        var ifMatch = html.match(/src="(https?:\/\/[^"]*(?:player|embed|iframe)[^"]*)"[^>]*>/i);
        if (ifMatch) {
          episodes.push({ title: 'Сезон ' + seasonNum, episode: 0, iframe: ifMatch[1], type: 'iframe' });
          return episodes;
        }
        var dRe = /data-(?:file|src|playlist|href)="([^"]*\.(?:m3u8|mp4)[^"]*)"/gi;
        var dm, epNum = 1;
        while ((dm = dRe.exec(html)) !== null) {
          episodes.push({ title: 'Серия ' + epNum, episode: epNum, file: dm[1], type: 'file' }); epNum++;
        }
        if (!episodes.length) {
          var epRe = /href="(\/serial[^"]*episode-(\d+)[^"]*)"[^>]*>([^<]+)</g;
          var em;
          while ((em = epRe.exec(html)) !== null) {
            episodes.push({ title: em[3] || ('Серия ' + parseInt(em[2])), episode: parseInt(em[2]), url: em[1], type: 'page' });
          }
        }
        return episodes;
      }
      ...
      function parseQualities(fStr) {
        var result = [];
        var re = /\[(\d+p)\](https?:\/\/[^\[]+)/gi;
        var m;
        while ((m = re.exec(fStr)) !== null) result.push({ title: m[1], url: m[2].trim() });
        if (!result.length && fStr && fStr.indexOf('://') !== -1) result.push({ title: 'Auto', url: fStr.trim() });
        return result;
      }
    ...
    Lampa.Params.trigger('online_mod_proxy_seasonvar', false);
    ...
    function initLang() {
      ...
      Lampa.Lang.add({
        ...
        online_mod_seasonvar_host: {
          ru: '\u0417\u0435\u0440\u043a\u0430\u043b\u043e Seasonvar (\u0445\u043e\u0441\u0442)',
          en: 'Seasonvar mirror (host)'
        },
        online_mod_seasonvar_clear: {
          ru: '\u041e\u0447\u0438\u0441\u0442\u0438\u0442\u044c \u043a\u044d\u0448 Seasonvar',
          en: 'Clear Seasonvar cache'
        },
    ...
    function initSettings() {
      ...
      template += "\n        <div class=\"settings-param selector\" data-name=\"online_mod_proxy_seasonvar\" data-type=\"toggle\">\n            <div class=\"settings-param__name\">#{online_mod_proxy_balanser} Seasonvar</div>\n            <div class=\"settings-param__value\"></div>\n        </div>";
