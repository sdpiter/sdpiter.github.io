...
(function () {
    ...
    function seasonvar(component, _object) {
      ...
      var baseUrl = 'https://www.seasonvar-enter.ru';
      var apiUrl = baseUrl + '/plStat.php';

      this.search = function(_object, kinopoisk_id, data) {
        object = _object;
        select_title = object.search || object.movie.title;
        if (!select_title) return;

        var searchUrl = baseUrl + '/search/?q=' + encodeURIComponent(select_title);

        component.loading(true);
        network.clear();
        network.timeout(15000);
        network.silent(searchUrl, function(html) {
          var matches = html.match(/serial-(\d+)-[^"]+\.html/g);
          if (matches && matches[0]) {
            var serialLink = baseUrl + '/' + matches[0];
            loadSerial(serialLink);
          } else {
            component.emptyForQuery(select_title);
          }
        }, function(a, c) {
      ...
      function parse(html) {
        var serialMatch = html.match(/data-id-serial="(\d+)"/);
        if (!serialMatch) {
          component.emptyForQuery(select_title);
          return;
        }

        extract.serial_id = serialMatch[1];

        var seasonMatch = html.match(/data-id-season="(\d+)"/);
        extract.id_season = seasonMatch ? seasonMatch[1] : 0;

        extract.translates = {};
        extract.playlists = {};

        var playlistRegex = /pl\[(\d+)\]\s*=\s*"([^"]+)"/g;
        var match;
        while ((match = playlistRegex.exec(html)) !== null) {
          extract.playlists[match[1]] = baseUrl + match[2];
        }

        var translateRegex = /<li[^>]*data-translate="(\d+)"[^>]*>([^<]+)<\/li>/g;
        while ((match = translateRegex.exec(html)) !== null) {
          extract.translates[match[1]] = match[2].trim();
        }

        if (extract.translates['0']) {
          select_translate = '0';
        } else {
          var keys = Object.keys(extract.translates);
          select_translate = keys[0] || '0';
        }

        var episodesMatch = html.match(/var\s+arEpisodes\s*=\s*({[\s\S]*?});/);
        if (episodesMatch) {
          try {
            extract.episodes = JSON.parse(episodesMatch[1]);
          } catch(e) {
            extract.episodes = {};
          }
        } else {
        ...
        loadPlaylistForTranslate(select_translate);
      }

      function loadPlaylistForTranslate(transId) {
        if (!extract.playlists[transId]) {
          component.emptyForQuery(select_title);
          return;
        }

        var playlistUrl = extract.playlists[transId];

        network.clear();
        network.timeout(15000);
        network.silent(playlistUrl, function(responseText) {
          try {
            var playlistData = JSON.parse(responseText);
            if (Array.isArray(playlistData)) {
              extract.fileIds = {};
              extract.playlistData = playlistData;
              playlistData.forEach(function(item, index) {
                extract.fileIds[(index + 1).toString()] = item.vars;
              });
            }
          } catch(e) {
            component.emptyForQuery(select_title);
      ...
      this.filter = function(type, a, b) {
        ...
        if (a.stype == 'voice') {
          choice.voice_name = filter_items.voice[b.index];
          select_translate = filter_items.voice_ids[b.index];
          component.reset();
          loadPlaylistForTranslate(select_translate);
          component.saveChoice(choice);
        }
      ...
      function filter() {
        var voiceNames = [];
        var voiceIds = [];
        for (var id in extract.translates) {
          voiceNames.push(extract.translates[id]);
          voiceIds.push(id);
        }

        if (select_translate !== 0) {
          var idx = voiceIds.indexOf(String(select_translate));
          if (idx !== -1) choice.voice = idx;
        }
      ...
      function filtred() {
        ...
        if (extract.playlistData && Array.isArray(extract.playlistData)) {
          extract.playlistData.forEach(function(item, index) {
            var episodeNum = index + 1;
            var title = (item.title || '').replace(/<br>/g, ' ').trim();
      ...
      function getStream(element, call, error) {
        var fileId = element.media ? element.media.file_id : '';
        var transId = element.media ? element.media.trans_id : select_translate;

        if (!extract.serial_id || !extract.id_season || !fileId) {
          if (error) error('Отсутствуют данные для получения видео');
          return;
        }

        var postData = 'serial_id=' + extract.serial_id +
                      '&id_season=' + extract.id_season +
                      '&file_id=' + fileId +
                      '&idTrans=' + transId +
                      '&numFile=1' +
                      '&svid1=-1';
        ...
        network.clear();
        network.timeout(15000);
        network.post(apiUrl, postData, function(response) {
          try {
            var data = JSON.parse(response);
            if (data && data[0] && data[0].file) {
              var encodedUrl = data[0].file;
              if (encodedUrl.indexOf('#2L') === 0) {
              ...
              try {
                ...
                if (error) error('Ошибка декодирования видео');
              }
            } else {
              if (error) error('Видео не найдено');
            }
          } catch(e) {
            if (error) error('Ошибка обработки ответа');
          }
        }, function(a, c) {
          if (error) error(network.errorDecode(a, c));
        }, {
      ...
      function append(items) {
        ...
        items.forEach(function(element) {
          ...
          var view = Lampa.Timeline.view(hash);
          var item = Lampa.Template.get('online_mod', element);
          var hash_file = Lampa.Utils.hash(element.season ? [element.season, element.season > 10 ? ':' : '', element.episode, object.movie.original_title, element.title, 'seasonvar'].join('') : object.movie.original_title + element.quality + 'seasonvar');
          element.timeline = view;
          item.append(Lampa.Timeline.render(view));
    ...
    function initStorage() {
      ...
      Lampa.Storage.set('online_mod_proxy_seasonvar', 'false');
